<template>
  <div class="space-y-6">
    <!-- Feedback Alerts -->
    <div v-if="outboundStore.errorMessage" class="p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-xs md:text-sm text-rose-600 dark:text-rose-400 flex justify-between items-center shadow-sm">
    ...
    <div v-if="outboundStore.successMessage" class="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-xs md:text-sm text-emerald-600 dark:text-emerald-400 flex justify-between items-center shadow-sm">
      <div class="flex items-center space-x-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
      ...
      <div class="p-5 md:p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4 shadow-sm transition-colors">
        ...
        <div class="p-6 md:p-8 border-2 border-dashed rounded-2xl text-center space-y-3 transition-colors" :class="photoPreview ? 'border-emerald-500/40 bg-emerald-500/5' : 'border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950'">
          <div class="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center mx-auto text-slate-500">
          ...
          <button 
            type="button" 
            @click="takePhoto"
            class="px-4 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 active:scale-95 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 text-xs font-semibold rounded-xl transition cursor-pointer shadow-2xs flex items-center justify-center space-x-1.5 mx-auto"
          >
        ...
        <div class="text-[11px] text-slate-400 space-y-1 bg-slate-50 dark:bg-slate-950 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
      ...
      <div class="p-5 md:p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4 shadow-sm transition-colors">
        <div class="border-b border-slate-100 dark:border-slate-800 pb-2 flex justify-between items-center">
          ...
          <span class="text-xs font-mono text-blue-600 dark:text-blue-400 font-semibold">BAST Digital</span>
        </div>
        ...
        >
          ...
          <span>{{ outboundStore.isLoading ? 'Memproses...' : 'Kirim Bukti POD & Rekam BAST' }}</span>
        </button>
      </div>
