<template>
  <div class="space-y-6">
    <!-- Feedback Alerts -->
    <div v-if="gatePassStore.errorMessage" class="p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-xs md:text-sm text-rose-600 dark:text-rose-400 flex justify-between items-center shadow-sm">
    ...
    <div v-if="gatePassStore.successMessage" class="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-xs md:text-sm text-emerald-600 dark:text-emerald-400 flex justify-between items-center shadow-sm">
      <div class="flex items-center space-x-2">
        <span>✓</span>
    ...
    <div class="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 transition-colors">
      ...
      <!-- Mode Switcher Tabs -->
      <div class="flex p-1 bg-slate-100 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 self-stretch md:self-auto min-w-[280px]">
        <button 
          type="button" 
    ...
    <form v-if="activeTab === 'departure'" @submit.prevent="handleDeparture" class="space-y-6">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <div class="p-5 md:p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4 shadow-sm transition-colors">
          ...
          <div class="space-y-1.5">
            ...
            <select 
              ...
              required 
              class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-3.5 py-3 text-sm text-slate-900 dark:text-slate-100 font-medium focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none"
            >
          ...
          <div class="space-y-1.5">
            ...
            <div class="flex space-x-2">
              <input 
                ...
                class="flex-1 bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 dark:text-slate-100 font-mono font-bold focus:border-blue-500 focus:outline-none"
              />
              <button 
                type="button" 
                @click="triggerQrScan" 
                class="px-4 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-200 transition flex items-center space-x-1.5 cursor-pointer"
              >
        ...
        <div class="p-5 md:p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4 shadow-sm transition-colors">
          ...
          <div class="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl space-y-1.5">
          ...
          <div class="space-y-1.5">
            ...
            <div class="grid grid-cols-4 gap-2 font-mono text-xs">
              <button 
                ...
                class="py-2.5 rounded-xl border text-center font-semibold transition cursor-pointer"
          ...
          <button 
            ...
            class="w-full py-3.5 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-semibold rounded-xl shadow-sm transition flex items-center justify-center space-x-2 disabled:opacity-50 text-sm cursor-pointer mt-2"
          >
    ...
    <div v-else class="space-y-4">
      ...
      <div v-if="departedLogs.length === 0" class="p-12 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm">
        <div class="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center mx-auto mb-3 text-slate-500">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 lg:gap-6">
        <div 
          ...
          :key="log.id" 
          class="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-3.5 shadow-sm transition-all hover:shadow-md flex flex-col justify-between"
        >
          <div>
            ...
            <div class="mt-3 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400 font-mono space-y-0.5">
          ...
          <div class="pt-2 space-y-2 border-t border-slate-100 dark:border-slate-800">
            <div class="flex space-x-2">
              <input 
                ...
                class="flex-1 bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-3 py-2 text-xs md:text-sm text-slate-900 dark:text-slate-100 font-mono font-bold focus:border-blue-500 focus:outline-none"
              />
              <button 
                type="button" 
                @click="handleReturn(log)"
                :disabled="gatePassStore.isLoading"
                class="px-4 py-2 bg-blue-600 hover:bg-blue-500 active:bg-blue-700 text-white text-xs font-bold rounded-xl shadow-sm disabled:opacity-50 transition cursor-pointer"
              >
                Gate In
              </button>
            </div>
