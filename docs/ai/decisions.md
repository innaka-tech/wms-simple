...
### ADR-12: PostgreSQL Resmi di Semua Environment (SQLite Dihapus dari Runtime)
- **Decision:** (v4.0.0) Runtime backend **wajib PostgreSQL** dari global DB stack (`postgres:16-alpine`, `127.0.0.1:5432`, DB `wms_simple_db`; test: `wms_simple_test_db`) — sesuai AGENTS.md rule 1. Layer `db.ts` kini wrapper tipis `node-postgres` (pg Pool asli, transaksi & FOR UPDATE native). SQLite (`node:sqlite` + `sqlite-db.ts`) **dihapus** dari runtime; schema dikelola `pg-schema.ts` (DDL + seed idempotent, migrasi guarded). Dialek lama dipetakan di `normalizeSql`: `uuid_generate_v4()` → `gen_random_uuid()` (core PG13+), `rowid` → `ctid`. Parameter binding PostgreSQL native `$1..$n` — menghilangkan kelas bug translasi placeholder SQLite.
- **Rationale:** Satu engine = satu sumber kebenaran (prinsip Engine Tunggal databases/README.md). Audit 2026-09-07 membuktikan lapisan translasi SQLite menyembunyikan bug P0 (PK NULL, binding salah urutan) yang lolos dari test mock — jalur dev dan produksi kini identik.
